...
//var serverRoot = 'http://localhost/NetYog/public';
//==================================================
...
window.NetYog = {
    ...
    addNewList : function(){
        ...
        if(listTitle!=''){  
            var newList = new NetYog.ListViewModel();
            newList.title(listTitle);
            NetYog.CurrentBordViewModel.lists.push(newList);
    ...
    addNewCard: function(){
        ...
        if(cardDescription.val() != ''){
            ...
            var newCard = new NetYog.CardViewModel();
            var title = cardDescription.val();
            var listId = NetYog.CurrentBordViewModel.lists()[listIndex].id();
            newCard.title(title);
            var len = NetYog.CurrentBordViewModel.lists()[listIndex].cards().length;
            NetYog.CurrentBordViewModel.lists()[listIndex].cards.splice( len + 1, 0, newCard);
    ...
    onServerDataReceive: function(data){
        ...
        NetYog.CurrentBordViewModel = ko.mapping.fromJS(data);
        var board = document.getElementById('board');
        ko.applyBindings(NetYog.CurrentBordViewModel, board);
        self.setupCardAdder();
        self.setPreferredContainerWidth();        
    ...
    addListContextMenuTo: function(ctx){
        ...
        menu.find('[name="delete"]').click(function(){
            var list = NetYog.CurrentBordViewModel.lists()[self.getCurrentListIndex(this)];
            var id = list.id();
            NetYog.CurrentBordViewModel.lists.remove(list);
            menu.addClass('hide');
            self.setPreferredContainerWidth();
    ...
    hideTitleInput : function(target){
        ...
        var listId = NetYog.CurrentBordViewModel.lists()[self.getCurrentListIndex(target)].id();
    ...
    addCardDialog: function(ctx){
        var list = NetYog.CurrentBordViewModel.lists()[self.getCurrentListIndex(ctx)];
        var index = $(ctx).index();
...
ko.bindingHandlers.uiSortableCards = {
    init: function (element, valueAccessor, allBindingsAccesor, context) {
        ...
        $element.sortable({
            ...
            start: function(event, ui){
                ...
                oldList = NetYog.getCurrentListIndex(ui.item.parent());
            },
            stop : function(event, uib){
                newIndex = ko.utils.arrayIndexOf(uib.item.parent().children(), uib.item[0]);
                var newList = NetYog.getCurrentListIndex(uib.item);
                var srcListModel = NetYog.CurrentBordViewModel.lists()[oldList];
                var moveCardSrc = srcListModel.cards.splice(oldIndex,1)[0];                
                NetYog.CurrentBordViewModel.lists()[newList].cards.splice(newIndex, 0,moveCardSrc);

                // remove duplicates caused by jquery ui sortable.
                uib.item.remove();
                var nid = NetYog.CurrentBordViewModel.lists()[newList].id();
    ...
    update: function (element, valueAccessor, allBindingsAccesor, context) {
        ...
        $element.click(function(event){
            ...
            NetYog.addCardDialog($(event.target).parent());
        });
    }
    ...
    init: function (element, valueAccessor, allBindingsAccesor, context) {
        ...
        $element.sortable({
            ...
            stop : function(event, uib){
                ...
                newIndex = ko.utils.arrayIndexOf(uib.item.parent().children(), uib.item[0]);                
                var listParent = NetYog.CurrentBordViewModel.lists();
...
NetYog.CardViewModel = function(){
    this.title = ko.observable();
    this.color = ko.observable('');
    this.description = ko.observable('');
};

NetYog.BoardViewModel = function(){
    this.lists = ko.observableArray();
};

NetYog.ListViewModel = function (){
    this.title = ko.observable();
...
$(document).ready(function(){
    NetYog.initialize();
    window.onresize = NetYog.onWindowResize;
